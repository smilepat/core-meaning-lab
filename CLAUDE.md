...
- **`shared/core30.ts` 는 손으로 고치지 않는다.** 프로토타입에서 자동 추출된 파일이다.
  ...
  졸업시킨 뒤 이 규칙을 지운다.
- **API 키는 서버에만 둔다.** `ANTHROPIC_API_KEY` 를 클라이언트 코드나 `VITE_` 접두어
...
- 스택: Vite + React + TypeScript (client) / Express + Anthropic SDK (server).
- 채점 모델: `claude-opus-5`, 구조화 출력(`output_config.format`) 으로 형식 강제.
- `ANTHROPIC_API_KEY` 가 없으면 자동으로 오프라인 폴백 채점으로 동작한다 — 앱은 멈추지 않는다.
