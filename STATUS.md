...
2. 브라우저에서 5단계를 직접 돌려 UI 확인. 특히 **티어 탭 + 최우선 30 순위 표시**는
   화면으로 처음 나온 것이라 아직 사람 눈으로 안 봤다.
3. 단어 확장. 3군(추상 라틴계)이 수능 지문에서 가장 효용이 크므로 여기부터 늘리는 게 낫다.
