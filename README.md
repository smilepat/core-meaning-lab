...
안내를 닫으면 **제호**(`Masthead.tsx`)가 페이지 맨 위에 남는다 — sticky 헤더보다 **위**라
열면 가장 먼저 보이고, 스크롤하면 밀려 올라가며 그 자리를 검색·군 탭이 차지한다.
안내는 한 번 닫으면 다시 안 뜨므로 앱의 전제는 매번 보이는 자리에도 있어야 한다.
...
`nav` 는 **`position: sticky`** 다. `fixed` 였을 때는 뷰포트 맨 아래에 못박혀, 세로가 긴
화면에서 카드와 한참 떨어졌다 — 학습자가 **제시만 보고 의미확장이 있다는 걸 모를 수 있었다.**
sticky 는 내용이 짧으면 카드 바로 밑에 따라붙고, 길면 스크롤 중 하단에 머문다.
(`main` 의 `min-height:60vh` 도 같은 이유로 뺐다. 짧은 내용을 화면 높이만큼 늘려 nav 를
저 아래로 밀고 있었다.)

그리고 **모든 단계에 다음으로 넘어가는 버튼**이 있다. 예전에는 STEP 1 에만 있어서
의미확장지도에서 가지를 다 펼친 뒤 어디로 갈지 단서가 없었다.
제시 → 의미확장 → 문맥복원 → 역방향 → 리포트가 버튼만 눌러도 이어진다.
